...
function Nav() {
  ...
  return (
    <div className='w-1/4 min-w-[250px] px-8 h-screen top-0 left-0 bottom-0 border-r border-[#efefef]'>
      ...
      <div className='flex bg-[#efefef] mb-14 group rounded-lg w-full focus-within:ring ring-offset-2 ring-sky-500 shadow-inner px-4 py-2 items-center '>
        <SearchIcon />
      ...
      <div onClick={() => { openCategory('/'); updateActiveElement(-1); }} className={` ${activeElement == -1 ? 'bg-[#efefef]' : 'bg-white'} flex h-[56px] drop-shadow-md justify-between cursor-pointer w-full px-6 py-5 items-center rounded-xl hover:bg-[#E7E7E7]`}>
      ...
      <div className='mt-12 px-2 w-full pb-10'>
        ...
        <button onClick={() => setShowAddForm(!showAddForm)} type={'button'} className='active:bg-[#8468CE] my-4 h-[56px]  rounded-[2.5rem] px-6 py-4 bg-[#6750A4] text-white hover:border-blue-300 w-full'>+ Add List</button>
