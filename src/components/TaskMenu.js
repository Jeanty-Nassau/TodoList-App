  ...
  return (
    <div className='flex-grow overflow-x-hidden overflow-y-scroll'>
      <div className='w-full flex justify-end '>
        <button onClick={() => setShowAddForm(!showAddForm)} className='active:bg-[#8468CE] flex cursor-pointer justify-end bg-[#6750A4] rounded-[2.5rem] px-5 py-3 mr-8 text-white  my-5'>
      ...
      <div className='flex justify-between items-center '>
        ...
        <div className='flex  pb-2 border-b'>
          <p className='mx-2 text-[#848484]'> {`${todos.filter((todo) => (todo.category == id && todo.completed === true)).length} Completed `}&middot;</p>
      ...
      {showAddForm && <form onSubmit={e => { handleSubmit(e) }} className='border border-slate-500 rounded-md absolute flex space-x-2 bg-slate-500 justify-center items-center w-96 h-32 p-4'>
        ...
        <div className='flex flex-col justify-start items-left my-4 bg-white rounded-xl px-4 py-8 shadow-md'>
        <ul className='w-full'>
